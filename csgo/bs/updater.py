...
class BSAuthAsyncRequest(grequests.AsyncRequest):

    def __init__(self, method, url, token_provider: TokenProvider, **kwargs):
    ...
    def send(self, **kwargs):
        ...
        self.url = self.url + f'&api_key={api_key}&code={token}'
...
class BSUpdater(Updater):
    ...
    def request_prices(self, items: Dict[str, float], per_page: int = 480) -> ItemPrices:
        prices = {}
        processing_requests = [
            self.request_inventory_on_sale(market_name, self.token_provider, ref_price, per_page=per_page)
            for market_name, ref_price in items.items()
        ]
        with tqdm(total=len(items)) as pbar:
            while processing_requests:
                tqdm.write(f'Got batch of {len(processing_requests)} request(s) to process')
                additional_requests = []
                for res in grequests.imap(processing_requests, size=10):
                    success = self.check_request(res, additional_requests, self.token_provider)
                    ...
                    if success:
                        ...
                        max_price = parse_qs(urlparse(original_req.url).query)['max_price'][0]
                        tqdm.write(f'Updated {item_name} {max_price} with {len(items)} items - '
                                   f'{page} ({res.elapsed.seconds}s)')

                        if page and len(items) == per_page:
                            self.add_next_page_request(res, additional_requests, self.token_provider, page)
                        else:
                            pbar.update(1)
    ...
    def request_sales(self, items: Set[str]) -> ItemSales:
        sales = {}
        processing_requests = [self.request_price_history(market_name, self.token_provider) for market_name in items]

        with tqdm(total=len(items)) as pbar:
            while processing_requests:
                tqdm.write(f'Got batch of {len(processing_requests)} request(s) to process')
                additional_requests = []
                for res in grequests.imap(processing_requests, size=5):
                    success = self.check_request(res, additional_requests, self.token_provider)
    ...
    @classmethod
    def check_request(cls, res: Response, requests_queue, token_provider: TokenProvider):
        if res.status_code in [429, 401]:
            ...
            if res.status_code == 429:
                time.sleep(3)
            requests_queue.append(BSAuthAsyncRequest('GET', res.request.url, token_provider))
            return False
        else:
    ...
    @classmethod
    def add_next_page_request(cls, res: Response, requests_queue, token_provider: TokenProvider, page: int):
        next_page = page + 1
        requests_queue.append(BSAuthAsyncRequest('GET', res.request.url + f'&page={next_page}', token_provider))

    @classmethod
    def request_inventory_on_sale(cls, market_name: str, token_provider: TokenProvider, ref_price: float,
                                  page: int = 1, per_page=480) -> grequests.AsyncRequest:
        max_price = f'{ref_price:.2f}' if ref_price else ''
        return BSAuthAsyncRequest('GET',
                                  f'{cls.prices_url}/?app_id=730&market_hash_name={market_name}'
                                  f'&is_souvenir=-1&max_price={max_price}&sort_by=price&order=asc'
                                  f'&page={page}&per_page={per_page}', token_provider)

    @classmethod
    def request_price_history(cls, market_name: str, token_provider: TokenProvider) -> grequests.AsyncRequest:
        return BSAuthAsyncRequest('GET', f'{cls.sales_url}/?app_id=730&market_hash_name={market_name}', token_provider)
    ...
    def get_prices_from_response(cls, res: Response) -> List[PriceEntry]:
        return [PriceEntry(i['market_hash_name'], float(i['price']), float(i['float_value']), i['item_rarity'],
                           item_id=i['item_id'])
